...
export default function AdminDashboardPage() {
  ...
  const [newCoverFile, setNewCoverFile] = useState(null); // NEW: For replacing cover image
  const [managingAlbum, setManagingAlbum] = useState(null);
  const [isSettingCoverId, setIsSettingCoverId] = useState(null); // NEW: Loading state for setting an existing image as cover
  ...
  // The original edit handler (for uploading a completely new file or changing text)
  const submitMediaEdit = async (e) => {
    e.preventDefault();
    try {
      const formData = new FormData();
      formData.append('id', editingMedia._id);
      formData.append('title', editingMedia.title);
      formData.append('description', editingMedia.description);
      formData.append('category', editingMedia.category);
      if (newCoverFile) {
        formData.append('newCover', newCoverFile);
      }
  ...
  // NEW: Handler to set an existing album image as the main cover
  const handleSetAsCover = async (mediaId, imageObj) => {
    if (!window.confirm("Set this photo as the new cover image?")) return;

    setIsSettingCoverId(imageObj.publicId);
    try {
      // NOTE: You will need to create this PUT endpoint in your Next.js API
      const res = await fetch('/api/media/cover', {
        method: 'PUT',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({
          mediaId: mediaId,
          newCoverUrl: imageObj.imageUrl,
          newCoverPublicId: imageObj.publicId
        })
      });
      const data = await res.json();

      if (data.success) {
        // Update local states so UI reflects changes instantly
        setManagingAlbum(data.data);
        setMediaItems(prev => prev.map(item => item._id === data.data._id ? data.data : item));
      } else {
        alert(data.error || "Failed to set new cover.");
      }
    } catch (error) {
      console.error("Failed to set cover", error);
      alert("An error occurred while setting the cover.");
    } finally {
      setIsSettingCoverId(null);
    }
  };
  ...
  return (
    <div className="min-h-screen bg-[#f8fafc] flex flex-col items-center pb-20">
      ...
      <div className="w-full bg-white border-b border-slate-200 px-4 sm:px-8 py-6 sticky top-0 z-40 shadow-sm">
        <div className="max-w-6xl mx-auto flex justify-between items-center">
          <div>
            ...
            <p className="text-xs text-slate-500 mt-1 font-bold uppercase tracking-widest">THE SYSTEM <span className="text-[#4b66c1]">|</span> Admin</p>
          </div>
        ...
        <div className="max-w-6xl mx-auto flex gap-6 mt-8 overflow-x-auto no-scrollbar border-b border-slate-100">
          {[
            ...
            <button
              ...
              className={`pb-3 text-sm font-bold tracking-widest uppercase transition-colors relative whitespace-nowrap ${activeTab === tab.id ? 'text-[#243465]' : 'text-slate-400 hover:text-slate-600'}`}
            >
              {tab.label}
              {activeTab === tab.id && <div className="absolute bottom-0 left-0 w-full h-1 bg-[#243465] rounded-t-lg"></div>}
            </button>
          ))}
        </div>
      </div>
      ...
      <div className="w-full max-w-6xl p-4 sm:p-8 mt-4 relative">
        ...
        {activeTab === 'upload' && (
          <form onSubmit={handleUploadSubmit} className="bg-white p-6 sm:p-10 rounded-3xl shadow-[0_8px_30px_rgb(0,0,0,0.04)] border border-slate-100 max-w-4xl mx-auto">
            ...
            <div className="mb-6">
              ...
              <input type="text" value={title} onChange={(e) => setTitle(e.target.value)} required className="w-full px-4 py-3.5 rounded-xl border border-slate-200 bg-slate-50 text-slate-900 focus:ring-2 focus:ring-[#4b66c1] focus:outline-none" placeholder="E.g., Youth Town Hall 2026" />
            </div>

            <div className="mb-6">
              <label className="block text-xs font-bold text-slate-500 uppercase tracking-widest mb-2 ml-1">Category</label>
              <select value={category} onChange={(e) => setCategory(e.target.value)} className="w-full px-4 py-3.5 rounded-xl border border-slate-200 bg-slate-50 text-slate-900 focus:ring-2 focus:ring-[#4b66c1] appearance-none font-medium focus:outline-none">
            ...
            <div className="mb-6">
              ...
              <textarea value={description} onChange={(e) => setDescription(e.target.value)} className="w-full px-4 py-3.5 rounded-xl border border-slate-200 bg-slate-50 text-slate-900 focus:ring-2 focus:ring-[#4b66c1] resize-none focus:outline-none" placeholder="Enter context..." rows="3" />
            </div>
            ...
            <div className="mb-6">
              ...
              {!previewUrl ? (
                <div className="relative">
                  ...
                  <div className="w-full px-4 py-10 rounded-xl border-2 border-dashed border-slate-300 bg-slate-50 flex flex-col items-center justify-center text-slate-500 hover:border-[#4b66c1]">
            ...
            {category === 'gallery' && previewUrl && (
              <div className="mb-10 p-6 bg-slate-50 border border-slate-200 rounded-2xl">
                ...
                <input type="file" accept="image/*" multiple onChange={handleAdditionalFilesChange} className="w-full text-sm text-slate-500 file:mr-4 file:py-2 file:px-4 file:rounded-full file:border-0 file:text-sm file:font-semibold file:bg-[#bcc8eb]/30 file:text-[#4b66c1] hover:file:bg-[#bcc8eb]/50 cursor-pointer" />
                {additionalFiles.length > 0 && <p className="mt-3 text-xs font-bold text-[#4b66c1] bg-[#bcc8eb]/20 inline-block px-3 py-1 rounded-full">{additionalFiles.length} additional photos selected.</p>}
              </div>
            )}

            {isUploading ? (
              <div className="flex justify-center items-center py-4 bg-slate-50 rounded-xl border border-slate-200">
                <div className="animate-spin rounded-full h-5 w-5 border-t-2 border-b-2 border-[#4b66c1]"></div><span className="ml-3 text-[#4b66c1] font-bold text-sm tracking-widest uppercase">Processing...</span>
              </div>
            ) : (
              <button type="submit" className="w-full bg-[#243465] text-white font-black tracking-widest uppercase py-4 rounded-xl hover:bg-[#172242] transition-all active:scale-[0.98]">Upload Media</button>
            )}
            {uploadMessage && <div className="mt-4 text-center text-sm font-bold text-[#4b66c1] uppercase tracking-widest">{uploadMessage}</div>}
          </form>
        )}
        ...
        {activeTab === 'library' && (
          <div className="bg-white rounded-3xl shadow-sm border border-slate-100 p-6 max-w-6xl mx-auto">
            <div className="p-2 border-b border-slate-100 flex justify-between items-center mb-6">
              ...
              <button onClick={fetchMedia} className="text-slate-400 hover:text-[#4b66c1] transition-colors p-2 rounded-full hover:bg-slate-50">
                <svg className={`w-5 h-5 ${isLoadingMedia ? 'animate-spin text-[#4b66c1]' : ''}`} fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M4 4v5h.582m15.356 2A8.001 8.001 0 004.582 9m0 0H9m11 11v-5h-.581m0 0a8.003 8.003 0 01-15.357-2m15.357 2H15" /></svg>
              </button>
            </div>

            {isLoadingMedia && mediaItems.length === 0 ? (
              <div className="p-12 flex justify-center items-center">
                <div className="animate-spin rounded-full h-8 w-8 border-t-2 border-b-2 border-[#4b66c1]"></div>
              </div>
            ) : mediaItems.length === 0 ? (
                ...
                {mediaItems.map(item => (
                  <div key={item._id} className="group border border-slate-200 rounded-2xl overflow-hidden shadow-sm flex flex-col bg-white hover:shadow-md transition-shadow">
                    ...
                    <div className="relative w-full aspect-video bg-slate-100">
                      ...
                      <span className="absolute top-2 left-2 bg-[#243465] px-2 py-1 rounded text-[10px] font-black uppercase tracking-widest shadow text-white">{item.category}</span>
                      {item.albumImages?.length > 0 && <span className="absolute top-2 right-2 bg-[#4b66c1] text-white px-2 py-1 rounded text-[10px] font-black uppercase shadow">+{item.albumImages.length}</span>}
                    </div>
                    ...
                    <div className="p-4 flex flex-col flex-1">
                      ...
                      <div className="mt-auto flex justify-between items-center border-t border-slate-100 pt-3">
                        <button onClick={() => copyToClipboard(item._id, item.imageUrl)} className="text-[10px] font-bold text-slate-500 uppercase tracking-widest hover:text-[#4b66c1] transition-colors">
                        ...
                        <div className="flex gap-2">
                          ...
                          <button onClick={() => { setEditingMedia(item); setNewCoverFile(null); }} className="text-blue-500 font-bold text-[10px] uppercase tracking-widest hover:text-blue-700 transition-colors">Edit</button>
          ...
          <div className="fixed inset-0 z-50 flex items-center justify-center bg-slate-900/80 backdrop-blur-sm p-4 overflow-y-auto">
            <div className="bg-white rounded-3xl p-6 md:p-8 w-full max-w-4xl shadow-2xl my-auto">
              <div className="flex justify-between items-center mb-6 border-b border-slate-100 pb-4">
                <h2 className="text-xl font-black uppercase tracking-tight text-[#243465]">Manage Album: {managingAlbum.title}</h2>
              ...
              <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 gap-4 mb-8">
                ...
                <div className="relative aspect-square rounded-xl overflow-hidden border-2 border-[#4b66c1]">
                  <img src={managingAlbum.imageUrl} className="w-full h-full object-cover" />
                  <div className="absolute bottom-0 w-full bg-[#4b66c1] text-white text-[10px] font-black uppercase tracking-widest text-center py-1">Cover (Main)</div>
                </div>
                ...
                {managingAlbum.albumImages && managingAlbum.albumImages.map((img) => (
                  <div key={img.publicId} className="relative aspect-square rounded-xl overflow-hidden border border-slate-200 group">
                    ...
                    {/* NEW: Loading overlay for this specific image when being set as cover */}
                    {isSettingCoverId === img.publicId ? (
                      <div className="absolute inset-0 bg-white/80 flex flex-col items-center justify-center z-10">
                        <div className="animate-spin rounded-full h-6 w-6 border-t-2 border-b-2 border-[#4b66c1] mb-2"></div>
                        <span className="text-[10px] font-bold text-[#4b66c1] uppercase tracking-widest">Applying...</span>
                      </div>
                    ) : (
                      <div className="absolute inset-0 bg-black/50 opacity-0 group-hover:opacity-100 transition-opacity flex flex-col items-center justify-center gap-2">
                        ...
                        <button
                          onClick={() => handleSetAsCover(managingAlbum._id, img)}
                          className="px-3 py-1.5 bg-white hover:bg-slate-100 rounded-full text-[#243465] text-[10px] font-black uppercase tracking-widest transform hover:scale-105 transition-all shadow-lg flex items-center gap-1"
                          title="Set as Cover"
                        >
              ...
              <div className="bg-slate-50 p-6 rounded-2xl border border-slate-200">
                ...
                <div className="flex flex-col sm:flex-row items-center gap-4">
                  <input type="file" accept="image/*" multiple onChange={handleAddPhotosToAlbum} className="w-full text-sm text-slate-500 file:mr-4 file:py-2.5 file:px-6 file:rounded-full file:border-0 file:text-sm file:font-bold file:uppercase file:tracking-widest file:bg-[#bcc8eb]/30 file:text-[#4b66c1] hover:file:bg-[#bcc8eb]/50 cursor-pointer transition-colors" />
                  {isUploading && (
                    <div className="flex items-center gap-2 shrink-0">
                      <div className="animate-spin rounded-full h-5 w-5 border-t-2 border-b-2 border-[#4b66c1]"></div>
                      <span className="text-xs font-bold text-[#4b66c1] uppercase tracking-widest">Uploading...</span>
                    </div>
                  )}
                </div>
              </div>
        ...
        {/* Edit Media Modal (Now Supports Cover Swap) */}
        {editingMedia && (
          <div className="fixed inset-0 z-50 flex items-center justify-center bg-slate-900/80 backdrop-blur-sm p-4">
            <div className="bg-white rounded-3xl p-8 w-full max-w-md shadow-2xl">
              <h2 className="text-xl font-black mb-6 uppercase tracking-tight text-[#243465]">Edit Details</h2>
              <form onSubmit={submitMediaEdit}>

                <label className="block text-xs font-bold text-slate-500 uppercase tracking-widest mb-2 ml-1">Change Cover Image (Optional)</label>
                <input type="file" accept="image/*,video/*" onChange={(e) => handleFileChange(e, 'editCover')} className="w-full mb-4 text-sm text-slate-500 file:mr-4 file:py-2 file:px-4 file:rounded-full file:border-0 file:text-xs file:font-bold file:bg-slate-100 file:text-slate-700 hover:file:bg-slate-200 cursor-pointer" />

                <label className="block text-xs font-bold text-slate-500 uppercase tracking-widest mb-2 ml-1">Title</label>
                <input type="text" value={editingMedia.title} onChange={e => setEditingMedia({ ...editingMedia, title: e.target.value })} className="w-full mb-4 px-4 py-3.5 rounded-xl border focus:ring-2 focus:ring-[#4b66c1] focus:outline-none" />

                <label className="block text-xs font-bold text-slate-500 uppercase tracking-widest mb-2 ml-1">Category</label>
                <select value={editingMedia.category} onChange={e => setEditingMedia({ ...editingMedia, category: e.target.value })} className="w-full mb-4 px-4 py-3.5 rounded-xl border font-medium focus:ring-2 focus:ring-[#4b66c1] focus:outline-none appearance-none">
                ...
                <label className="block text-xs font-bold text-slate-500 uppercase tracking-widest mb-2 ml-1">Description</label>
                <textarea value={editingMedia.description} onChange={e => setEditingMedia({ ...editingMedia, description: e.target.value })} className="w-full mb-8 px-4 py-3.5 rounded-xl border resize-none focus:ring-2 focus:ring-[#4b66c1] focus:outline-none" rows="3" />

                <div className="flex gap-4">
                  <button type="button" onClick={() => { setEditingMedia(null); setNewCoverFile(null); }} className="flex-1 py-3.5 font-bold text-slate-600 bg-slate-100 rounded-xl hover:bg-slate-200 transition-colors uppercase tracking-widest text-xs">Cancel</button>
                  <button type="submit" className="flex-1 py-3.5 font-bold text-white bg-[#4b66c1] rounded-xl hover:bg-[#243465] transition-colors uppercase tracking-widest text-xs shadow-lg shadow-[#4b66c1]/20 active:scale-[0.98]">Save Changes</button>
                </div>
              </form>
            </div>
          </div>
        ...
        {activeTab === 'news' && (
          <form onSubmit={handleNewsSubmit} className="bg-white p-6 sm:p-10 rounded-3xl shadow-sm border border-slate-100 max-w-4xl mx-auto">
            <div className="flex justify-between items-center mb-8 border-b border-slate-100 pb-4">
              <h2 className="text-xl font-bold text-[#243465]">{editingArticle ? 'Edit Campaign Update' : 'Publish Campaign Update'}</h2>
              {editingArticle && (
            ...
            <div className="mb-6">
              ...
              <input type="text" value={newsTitle} onChange={(e) => setNewsTitle(e.target.value)} required className="w-full px-4 py-3.5 rounded-xl border focus:ring-2 focus:ring-[#4b66c1] focus:outline-none" placeholder="Headline" />
            </div>

            <div className="grid grid-cols-1 md:grid-cols-2 gap-6 mb-6">
              <div>
                <label className="block text-xs font-bold text-slate-500 uppercase tracking-widest mb-2 ml-1">Category</label>
                <select value={newsCategory} onChange={(e) => setNewsCategory(e.target.value)} className="w-full px-4 py-3.5 rounded-xl border font-medium focus:ring-2 focus:ring-[#4b66c1] focus:outline-none appearance-none">
              ...
              <div className="flex items-center md:mt-8 ml-2 bg-slate-50 px-4 rounded-xl border border-slate-100">
                <input type="checkbox" id="featured" checked={newsFeatured} onChange={(e) => setNewsFeatured(e.target.checked)} className="w-5 h-5 text-[#4b66c1] rounded cursor-pointer" />
            ...
            <div className="mb-6">
              ...
              <textarea value={newsExcerpt} onChange={(e) => setNewsExcerpt(e.target.value)} required rows="2" className="w-full px-4 py-3.5 rounded-xl border resize-none focus:ring-2 focus:ring-[#4b66c1] focus:outline-none" placeholder="Summary for the feed..." />
            </div>
            <div className="mb-6">
              <label className="block text-xs font-bold text-slate-500 uppercase tracking-widest mb-2 ml-1">Full Content</label>
              <textarea value={newsContent} onChange={(e) => setNewsContent(e.target.value)} required rows="10" className="w-full px-4 py-3.5 rounded-xl border resize-none focus:ring-2 focus:ring-[#4b66c1] focus:outline-none" placeholder="Write the full article..." />
            </div>

            <div className="mb-10">
              <label className="block text-xs font-bold text-slate-500 mb-2 uppercase tracking-widest">Cover Image {editingArticle && <span className="text-[#4b66c1] lowercase tracking-normal">(Leave empty to keep current)</span>}</label>
              {!newsPreviewUrl ? (
                <div className="relative">
            ...
            {isPublishing ? (
              <div className="flex justify-center items-center py-4 bg-slate-50 rounded-xl border border-slate-200">
                <div className="animate-spin rounded-full h-5 w-5 border-t-2 border-b-2 border-[#4b66c1]"></div><span className="ml-3 text-[#4b66c1] font-bold text-sm uppercase tracking-widest">Saving...</span>
              </div>
            ) : (
              <button type="submit" className="w-full bg-[#243465] text-white font-black uppercase tracking-widest py-4 rounded-xl hover:bg-[#172242] transition-all active:scale-[0.98]">{editingArticle ? 'Save Changes' : 'Publish Article'}</button>
            )}
            {publishMessage && <div className="mt-4 text-center text-[#4b66c1] font-bold uppercase tracking-widest text-sm">{publishMessage}</div>}
          </form>
        )}
        ...
        {activeTab === 'manage-news' && (
          <div className="bg-white rounded-3xl shadow-sm border border-slate-100 p-6 max-w-4xl mx-auto">
            <div className="p-2 border-b border-slate-100 flex justify-between items-center mb-6">
              ...
              <button onClick={fetchArticles} className="text-slate-400 hover:text-[#4b66c1] transition-colors p-2 rounded-full hover:bg-slate-50">
                <svg className={`w-5 h-5 ${isLoadingArticles ? 'animate-spin text-[#4b66c1]' : ''}`} fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M4 4v5h.582m15.356 2A8.001 8.001 0 004.582 9m0 0H9m11 11v-5h-.581m0 0a8.003 8.003 0 01-15.357-2m15.357 2H15" /></svg>
              </button>
            </div>
              ...
              <div className="divide-y divide-slate-100 border border-slate-100 rounded-2xl overflow-hidden">
                {articles.map(article => (
                  <div key={article._id} className="p-6 flex flex-col sm:flex-row sm:justify-between sm:items-center gap-4 bg-slate-50/30 hover:bg-white transition-colors">
                    <div>
                      <p className="font-black text-slate-900 text-lg mb-1">{article.title} {article.featured && <span className="ml-2 text-[10px] bg-[#bcc8eb]/40 text-[#243465] px-2 py-0.5 rounded-full uppercase tracking-widest align-middle">Featured</span>}</p>
        ...
        {activeTab === 'inbox' && (
          <div className="bg-white rounded-3xl shadow-sm border border-slate-100 overflow-hidden max-w-4xl mx-auto">
            <div className="p-6 sm:p-8 border-b border-slate-100 flex justify-between items-center bg-slate-50/50">
              <h2 className="text-xl font-bold text-[#243465]">Voter Mandates</h2>
              <button onClick={fetchMessages} className="text-slate-400 hover:text-[#4b66c1] transition-colors p-2 rounded-full hover:bg-slate-50">
                <svg className={`w-5 h-5 ${isLoadingMessages ? 'animate-spin text-[#4b66c1]' : ''}`} fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M4 4v5h.582m15.356 2A8.001 8.001 0 004.582 9m0 0H9m11 11v-5h-.581m0 0a8.003 8.003 0 01-15.357-2m15.357 2H15" /></svg>
              </button>
            </div>
              ...
              <div className="divide-y divide-slate-100">
                {messages.map((msg) => (
                  <div key={msg._id} className="transition-colors">
                    <div onClick={() => handleMessageClick(msg)} className={`p-4 sm:p-6 cursor-pointer flex gap-4 items-center hover:bg-slate-50 transition-colors ${msg.status === 'unread' ? 'bg-[#bcc8eb]/20' : 'bg-white'}`}>
                      <div className="shrink-0 w-2 flex justify-center">{msg.status === 'unread' && <div className="w-2 h-2 bg-[#4b66c1] rounded-full shadow-[0_0_8px_rgba(75,102,193,0.8)]"></div>}</div>
                      <div className="flex-1 min-w-0 flex flex-col sm:flex-row sm:items-center justify-between gap-1 sm:gap-4">
                        <div className="flex flex-col min-w-0">
                          <p className={`truncate text-sm ${msg.status === 'unread' ? 'font-black text-[#243465]' : 'font-bold text-slate-700'}`}>{msg.name} <span className="font-normal text-slate-400 text-xs ml-2 hidden sm:inline">&lt;{msg.email}&gt;</span></p>
                    ...
                    {expandedMsgId === msg._id && (
                      <div className="p-6 sm:p-8 bg-slate-50 border-t border-slate-100 text-slate-700 text-sm leading-relaxed">
                        <div className="mb-6 flex flex-col sm:hidden pb-4 border-b border-slate-200">
                          ...
                          <a href={`mailto:${msg.email}`} className="text-[#4b66c1] font-bold">{msg.email}</a>
                        </div>
                        <p className="whitespace-pre-wrap">{msg.message}</p>
                        <div className="mt-8 pt-6 border-t border-slate-200 flex justify-end">
                          <a href={`mailto:${msg.email}?subject=Re: ${msg.subject}`} className="bg-[#243465] hover:bg-[#172242] text-white text-xs font-bold uppercase tracking-widest py-2.5 px-6 rounded-lg transition-colors shadow-sm">Reply via Email</a>
                        </div>
                      </div>
                    )}
                  </div>
